...
Implementacija algoritma \textit{AllEulerianCycles} se sastoji od velikog broj funkcija. Na taj način je posao koji obavlja ovaj algoritam podeljen na smislene celine. Na primer, funkcija \textit{in\_degree}, čija je implementacija prikazana na listingu \ref{lst:109}, računa ulazni stepen za zadati čvor. Ona je pomoćna funkcija funkcije koja izračunava ulazni i izlazni stepen za sve čvorove datog grafa. Graf je u implementaciji predstavljen mapom. Ključ mape je čvor grafa, a lista vrednosti ključa čvorovi sa kojima dati čvor gradi grane u grafu. Argumenti funkcije \textit{in\_degree} su lista svih ključeva grafa, graf \textit{graph}, čvor \textit{node} za koji se vrši računanje i ulazni stepen koji je na početku 0. Ulazni stepen čvora je broj grana koje ulaze u taj čvor, odnosno broj ključeva mape u čijem skupu vrednosti se nalazi taj čvor. Računanje se vrši tako sto se rekurzivno za svaki čvor grafa proverava da li se dati čvor nalazi u njegovoj listi vrednosti. Dohvatanje liste vrednosti za zadati čvor mape obavlja se funkcijom \textit{Map.get/2}, a provera se vrši funkcijom \textit{Enum.member?/2}. Ukoliko je povratna vrednost funkcije textit{Enum.member?/2} jednaka \textit{true}, vrednost ulaznog stepena \textit{degree} se uvećava za 1 i rekurzivno se poziva funckija \textit{in\_degree} za sledeći ključ mape. U suprotnom, samo se rekurzivno poziva funckija \textit{in\_degree} bez uvećavanja vredosti \textit{degree}. Kada se obradi i poslednji ključ mape, izlazi se iz rekurzije i vraća ulazni stepen \textit{degree}. Izračuvanavanje izlaznog stepena za zadati čvor vrši se izračunavanjem dužine liste vrednosti u mapi za zadati čvor pomoću funkcije \textit{Kernel.length/1}.

Izlazni stepen za dati čvor se računa jednostavno, utvrđivanjem dužine njegove liste vrednosti u mapi, a pomoću funkcije \textit{Kernel.length/1}.

\lstinputlisting[language=erlang,label={lst:109},caption=Funkcija \textit{in\_degree},captionpos=b]{109.ex}
...
% Algoritam \textit{DeBrujinGraph} je testiran nad jednim očitavanjem skupa očitavanja bakterije \textit{Staphylococcus aureus}. Očitavanje je dužine ... i može se videti na listingu ..., a parametar $k$ je 10. Rezultati se mogu videti na listingu...

% \lstinputlisting[language=erlang,label={lst:deBrujinOčitavanje},caption=Jedno očitavanje iz skupa očitavanja bakterije \textit{Staphylococcus aureus},captionpos=b]{eulerRezultati.ex}
...
% Algoritam \textit{AllEulerianCycles} je testiran nad rezultatom izvršavanja algoritma \textit{DeBrujinGraph} prikazanom na listingu \ref{lst:deBrujinOčitavanje}, pri čemu je format ulaznog fajla prilagođen. Nakon izvršavanja algoritma, dobija se lista kontiga koja je prikazana na listingu \ref{lst:eulerRezultati}.
