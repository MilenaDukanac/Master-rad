...
\lstinputlisting[language=erlang,label={lst:printArgs},caption=Primer definisanja modula i funkcije u okviru njega,captionpos=b]{odstampaj_argumente.ex}

\lstinputlisting[language=erlang,label={lst:primerPokretanjaHelloWorld},caption=Primer kompajliranja fajla i pokretanja funkcije,captionpos=b]{primerPokretanjaHelloWorld.ex}
...
\lstinputlisting[language=erlang,label={lst:osmi},caption=Izdvajanje elementa torke sa indeksom jedan,captionpos=b]{osmi.ex}
...
% Sa računarske strane genom predstavlja nisku nad azbukom $\{A, C, G, T\}$.
...
daje jedinstvenu poziciju. Ta funkcija naziva se \textbf{heš funkcija}, a tabela koja se koristi u tom
postupku zove se \textbf{heš tabela}. Kada je $k$ malo (npr. manje od 10), u procesu brojanja k-mera koristi se \textbf{savršeno heširanje}. Savršeno heširanje garantuje da neće doći do \textbf{kolizije}\footnote{Izraz koji potiče od latinske reči \textit{collisio} i znači sudar, sukob. U ovom kontekstu, moguće je da heš funkcija za dva različita k-mera da istu vrednost, te kažemo da su ta dva k-mera u koliziji, tj. sukobu.}. To je moguće kada se tačno zna koji skup ključeva će biti heširan prilikom dizajniranja heš funkcije. Svaki k-mer $z$ može biti kodiran kao 2k-bitni binarni ceo broj \textit{b(z)} zamenom $A$, $C$, $G$ i $T$ u $z$ sa $00$, $01$, $10$, $11$, respektivno. Tako se izgrađuje tabela $Count[0..4^k - 1]$ veličine $4^k$ u kojoj svaka ulazna vrednost $Count[b(z)]$ čuva frekvenciju k-mera $z$ u skupu $Z$. Preciznije, prvo se vrši inicijalizacija svake ulazne vrednosti u $Count[0..4^k - 1]$ na 0. Zatim se iterativno skenira svaki k-mer $z$ iz $Z$ i uvećava $Count[b(z)]$ za 1. Na kraju, sve ulazne vrednosti različite od nule u $Count[]$ predstavljaju k-mere koji se pojavljuju u $Z$ kao i broj njihovih pojavljivanja.
...
U drugoj fazi se obavlja stvarno brojanje. Vrši se skeniranje k-mera iz $Z$ jedan po jedan. Za svaki k-mer $z$ iz $Z$, ako se $z$ pojavljuje u heširanom ulazu $H[i]$, onda se uvećava $Count[i]$ za jedan.
...
% Osnovna ideja je da se ulazna lista k-mera podeli na više manjih podlista nad kojima će se pokrenuti algoritam \textit{JellyFish} i da se rezultati pojedinačnih pokretanja objedine i prikažu kao krajnji rezultat. Ovakva podela je izuzetno važna u situacijama kada se zbog nedovoljne količine memorije kompletni podaci ne mogu istovremeno obrađivati.

% Urađene su dve implementacije ovog algoritma. Prva prati osnovnu ideju koja je u ovom poglavlju predstavljena, a druga koristi implementaciju algoritma \textit{JellyFish} sa mapom i njenim mehanizmom za razrešavanje kolizija. U aplikaciji se koristi druga implementacija, jer je neuporedivo efikasnija.
...
\lstinputlisting[language=erlang,label={lst:jellyfishRezultatiParalelno},caption=Rezultati izvršavanja algoritma \textit{JellyFish},captionpos=b]{jellyfishRezultatiParalelno.ex}
...
\lstinputlisting[language=erlang,label={lst:dskRezultati},caption=Rezultati izvršavanja algoritma \textit{DSK},captionpos=b]{dskRezultati.ex}
...
\lstinputlisting[language=erlang,label={lst:debrujinRezultati},caption=Rezultati izvršavanja algoritma \textit{DeBrujinGraph},captionpos=b]{debrujinRezultati.ex}
...
\lstinputlisting[language=erlang,label={lst:eulerRezultati},caption=Rezultati izvršavanja algoritma \textit{AllEulerianCycles},captionpos=b]{eulerRezultati.ex}
...
% Uz pomoć algoritma \textit{DSK} kompletirana je obrada čitavog skupa k-mera dužine deset bakterije \textit{Staphylococcus aureus} i dobijen je jedan izlazni fajl sa rezultatima. Algoritam \textit{DSK} kao argumente prima skup k-mera i veličinu fajla koju algoritam \textit{JellyFish} može obraditi i u toku svog izvršavanja koristi algoritam \textit{JellyFish} za obradu podskupova (veličine zadatog argumenta) datog skupa k-mera. Zbog toga, vreme izvršavanja algoritma \textit{DSK} predstavlja zbir vremena potrebnog za izvršavanje algoritma \textit{JellyFish} na podskupovima datog skupa k-mera i vremena potrebnog za objedinjavanje rezultata izlaznih fajlova.
...
% Sa dijagrama se može videti da je vreme izvršavanja algoritma \textit{DSK} malo veće od vremena izvršavanja algoritma \textit{JellyFish}, a zbog dodatnog vremena potrebnog za objedinjavanje rezultata izlaznih fajlova. Algoritam \textit{JellyFish} je izvršavao obradu čitavog skupa k-mera, pri čemu su rezultati bili razdeljeni u osam različitih fajlova, za 93 sekunde, dok je algoritam \textit{DSK} obradio isti skup k-mera i rezultate objedinio u jedan fajl za 109 sekundi.
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\textwidth]{dijagram_izvrsavanja_euler.PNG}
