...
Na osnovu preuzetog materijala sa adrese \cite{GIT}, potrebno je napraviti projekat u \textit{PyCharm}-u i u okviru njega fajl sa ekstenzijom \textit{.py} koji je glavni za projekat. Paket \textit{\textbf{Tkinter}} je iskorišćen za implementaciju grafičkog korisničkog intefrejsa. Da bi korišćenje njegovih funkcionalnosti bilo moguće neophodno je na početku glavnog fajla navesti komandu \textit{from tkinter import *}. Zatim, u okviru projekta se nalaze izvršni fajlovi implementiranih algoritama (\textit{jellyFish}, \textit{dsk},\textit{debrujinGraph}, \textit{allEulerianCycles}). Tekstualni fajlovi iz kojih se čitaju ulazni parametri za algoritme se biraju pomoću posebnog dijaloga. Nakon izvršavanja svakog algoritma, kreira se novi tekstualni fajl unutar projekta, koji nosi naziv tog algoritma i u kome su smešteni rezultati njegovog izvršavanja. Izgled ulaznih i izlaznih fajlova za svaki pojedinačni algoritam biće opisan u poglavlju \ref{odeljak:Rezultati}.
