...
Minimizacija broja pristupa disku u implementaciji ovog algoritma nije ostvarena, jer u Elixir-u ne postoji kontrola za to, već je algoritam implementiran samo prateći osnovnu ideju. Zapravo, urađene su dve implementacije ovog algoritma. Prva prati osnovnu ideju koja je u ovom poglavlju predstavljena, a druga koristi implementaciju algoritma \textit{JellyFish} sa mapom i njenim mehanizmom za razrešavanje kolizija. U aplikaciji se koristi druga implementacija, jer je neuporedivo efikasnija.
...
\textbf{Algoritam \textit{DSK}} - Ulazni fajl \textit{DSKData.txt} za algoritam \textit{DSK} ima isti format kao i ulazni fajl za algoritam \textit{JellyFish}, samo što u poslednjoj liniji sadrži i broj koji predstavlja maksimalnu veličinu ulaznog fajla izraženu u kilobajtima koju algoritam \textit{JellyFish} može da obradi s obzirom na dostupnu memoriju. Primer ulaznog fajla može se videti na listingu \ref{lst:dskUlazniFajl}.
...
\lstinputlisting[language=erlang,label={lst:eulerUlazniFajl},caption=Primer ulaznog fajla za algoritam \textit{AllEulerianCycles} algoritma,captionpos=b]{allEulerianCyclesUlazniFajl.ex}
...
Izvršeno je eksperimentalno merenje i utvrđena je granica veličine podataka, odnosno broj k-mera koji mogu biti obrađeni algoritmom \textit{JellyFish} na datoj mašini. Ta granica iznosi 120KB, odnosno oko 11300000 k-mera (u našem slučaju k-meri su dužine 10). Vreme za koje ovaj broj k-mera biva obrađen paralelnim izvršavanjem je u proseku oko 11 sekundi, dok je u slučaju sekvencijalnog izvršavanja prosečna brzina 21 sekund.

Algoritmom \textit{JellyFish} je izvršena obrada svih 90403620 k-mera dužine 10 ekstarhovanih iz 462036 očitavanja različite dužine bakterije \textit{Staphylococcus aureus}. Početni skup k-mera je najpre podeljen na podskupove koji se mogu obraditi sa dostupnom memorijom. Kako je memorijska granica za računar na kojem je vršena obrada 120KB, to je skup svih očitavanja bio podeljen na osam podskupova. Potom je svaki podskup obrađen paralelno, odnosno izvršeno je dalje deljenje svakog podskupa na četiri dela (jer računar poseduje četiri procesora) i na svakom delu je paralelno pokrenut algoritam \textit{JellyFish}. Nakon završetka obrade, generisano je osam izlaznih fajlova sa rezultatima. Ove rezultate je moguće sabrati i od njih formirati krajnji izlazni fajl koji sadrži broj pojavljivanja za svaki k-mer dužine 10 iz skupa svih k-mera bakterije.
...
\textbf{Algoritam \textit{DSK}} - Uz pomoć algoritma \textit{DSK} kompletirana je obrada čitavog skupa k-mera dužine 10 bakterije \textit{Staphylococcus aureus} i dobijen je jedan izlazni fajl sa rezultatima. Algoritam \textit{DSK} kao argumente prima skup k-mera i veličinu fajla koju algoritam \textit{JellyFish} može obraditi i u toku svog izvršavanja koristi algoritam \textit{JellyFish} za obradu podskupova (veličine zadatog argumenta) datog skupa k-mera. Zbog toga, vreme izvršavanja algoritma \textit{DSK} predstavlja zbir vremena potrebnog za izvršavanje algoritma \textit{JellyFish} na podskupovima datog skupa k-mera i vremena potrebnog za objedinjavanje rezultata izlaznih fajlova. Kako bi se ocenile performanse algoritma \textit{DSK}, merene su brzine njegovog izvršavanja uz postepeno povećavanje broja izlaznih fajlova koje je potrebno objediniti u jedan. Dobijeni rezultati se mogu videti na dijagramu sa slike \ref{fig:dijagramIzvrsavanjaDSK}.
...
\textbf{Algoritam \textit{AllEulerianCycles}} - Performanse algoritma \textit{AllEulerianCycles} merene su na ulaznim podacima koji su dobijeni kao rezultat izvršavanja algoritma \textit{DeBrujinGraph} za 1000, 2000 i 3000 očitavanja, pri čemu je parametar $k = 5$. Ulazni podaci su zapravo grafovi kreirani algoritmom \textit{DeBrujinGraph} zapisani u formatu potrebnom za algoritam \textit{AllEulerianCycles}. Na dijagramu sa slike \ref{fig:dijagramIzvrsavanjaEuler} može se videti zavisnost brzine izvršavanja od broja ulaznih grafova.
